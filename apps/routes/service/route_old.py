...
import polyline
import requests
from apps.routes import GoogleMapsRouteClient
from apps.routes.serializers.route import CreateRouteSerializer, PreviewRouteSerializer
from apps.routes.service.dijkstra import dijkstra
from apps.routes.service.kPowerFinder import kPowerFinder
...
def buildMapsRouteRequest(
    ...
    mapping = {
        "origin": {
            "location": {
                "lat_lng": {
                    "latitude": valid.get("originLat"),  # type: ignore
                    "longitude": valid.get("originLon"),  # type: ignore
                }
            }
        },
        "destination": {
            "location": {
                "lat_lng": {
                    "latitude": valid.get("destinationLat"),  # type: ignore
                    "longitude": valid.get("destinationLon"),  # type: ignore
                }
            }
        },
    }
